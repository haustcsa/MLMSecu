...
def train(model, train_loader, optimizer, scheduler, test_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        if accuracy > best_acc:
            #   修改模型名称
            torch.save(model.state_dict(), f"checkpoint/teacher/{args.dataset}-alexnet{args.suffix}.pt")
            best_acc = accuracy
...
def main():
    ...
    parser.add_argument('--max-accuracy', type=float, default=0.85)
    ...
    parser.add_argument('--steps', nargs='+', default=[0.1, 0.2, 0.3, 0.5, 0.8], type=float,
                        help="Percentage epochs at which to take next step")
    parser.add_argument('--suffix', type=str, default="")
    parser.add_argument('--data-root', type=str, default='data')
    parser.add_argument('--no-cuda', action='store_true', default=False,
                        help='disables CUDA training')
    parser.add_argument('--dataset', type=str, default='tinyimagenet', choices=['svhn', 'cifar10', 'mnist', 'cifar100', 'tinyimagenet'],
    ...
    args.num_classes = 10 if args.dataset in ['cifar10', 'svhn', 'mnist'] else 200 if args.dataset == 'tinyimagenet' else 100
    ###  设置要训练的模型  ###
    model = network.alexnet.AlexNet32(num_classes=args.num_classes)
    model = model.to(args.device)
    ##########################################
    model.load_state_dict(torch.load(f"checkpoint/teacher/tinyimagenet-alexnet.pt"))

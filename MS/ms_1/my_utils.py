...
def student_loss(s_logit, t_logit, args):
    ...
    if args.loss == "l1":
        return F.mse_loss(s_logit, t_logit.detach(), reduction="mean")
...
def get_model_preds_and_true_labels(model, loader, device="cuda"):
    ...
    with torch.no_grad():
        for (data, target) in loader:
            ...
            output = model(data)
            assert len(output.shape) == 2 or (len(output.shape) == 3 and isinstance(model, StudentEnsemble))
...
def get_classifier(classifier, pretrained=True, num_classes=10, ensemble_size=16, device=None):
    ...
    if classifier == "wrn-28-10":
    ...
    elif classifier == "alexnet32":
        if pretrained:
            raise ValueError("Cannot load pretrained alexnet32 from here")
        return network.alexnet.AlexNet32(num_classes=num_classes)
    elif classifier == "lenet":
        return network.lenet.LeNet5(num_classes=num_classes)
    elif classifier == "ensemble_alexnet32":
        if pretrained:
            raise ValueError("Cannot load pretrained ensemble_alexnet32 from here")
        return StudentEnsemble(ensemble_size=ensemble_size, num_classes=num_classes,
                               student_model="ensemble_alexnet32")
    elif classifier == "vgg16":
        if pretrained:
            raise ValueError("Cannot load pretrained vgg16 from here")
        return network.VGG.vgg16(num_classes=num_classes)
    elif classifier == "ensemble_vgg16":
        if pretrained:
            raise ValueError("Cannot load pretrained ensemble_vgg16 from here")
        return StudentEnsemble(ensemble_size=ensemble_size, num_classes=num_classes,
                               student_model="ensemble_vgg16")
    ...
    elif classifier == "ensemble_resnet34_8x":
        if pretrained:
            raise ValueError("Cannot load pretrained resnet34_8x from here")
        return StudentEnsemble(ensemble_size=ensemble_size, num_classes=num_classes,
                               student_model="ensemble_resnet34_8x")
        ...
        return StudentEnsemble(ensemble_size=ensemble_size, num_classes=num_classes,
                               student_model="ensemble_lenet5_half")
...
def smooth_one_hot(true_labels: torch.Tensor, classes: int, smoothing=0.2):
    """
    ...
    with torch.no_grad():
        ...
        # 将所有元素填充为非目标类的平滑概率。
        # 目标类别的概率是1 - smoothing，而其他非目标类别的概率是平均分配的，即smoothing / (classes - 1)
        true_dist.fill_(smoothing / (classes - 1))
        _, index = torch.max(true_labels, 1)

        # 将confidence值放入true_labels的对应位置

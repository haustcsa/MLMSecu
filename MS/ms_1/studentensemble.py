...
class StudentEnsemble(nn.Module):
    ...
    def __init__(self, ensemble_size: int = 16, num_classes: int = 10, student_model: str = "ensemble_resnet34_8x"):
        super(StudentEnsemble, self).__init__()
        ...
        elif student_model == 'ensemble_resnet34_8x':
            self.subnets = nn.ModuleList([network.resnet_8x.ResNet34_8x(num_classes) for i in range(ensemble_size)])
        elif student_model == 'ensemble_alexnet32':
            self.subnets = nn.ModuleList([network.alexnet.AlexNet32(num_classes) for i in range(ensemble_size)])
        elif student_model == 'ensemble_vgg16':
            self.subnets = nn.ModuleList([network.VGG.vgg16(num_classes = num_classes) for i in range(ensemble_size)])

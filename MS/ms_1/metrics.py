...
class _F:
    ...
    def __call__(self, preds, labels, by_class=False):
        ...
        if by_class:
            return self.metric(preds, labels, num_classes=num_classes, average=None, task="multiclass")
        assert preds.device == labels.device, f"Expected preds and labels on same device. preds:{preds.device} labels:{labels.device}"
        return self.metric(preds, labels, num_classes=num_classes, average=self.avg, task="multiclass")
...
def _get_class_distribution_quantiles(num_classes):
    if num_classes == 10:
        ...
        return [("Min", "Max", 0.0), ("2nd Worst Class", "2nd Best Class", 1.0 / 199.0)]
    else:
